...
export default function Translator() {
  ...
  const handleTranslateClick = async () => {
    ...
    try {
      ...
      // inputText is already trimmed to MAX_CHARS by onChange
  ...
  const handleSpeak = () => {
    ...
    else if (targetLang === "ur") utterance.lang = "ur-PK";
    ...
    else if (targetLang === "pt") utterance.lang = "pt-PT";
    ...
    else if (targetLang === "ko") utterance.lang = "ko-KR";
    else if (targetLang === "tr") utterance.lang = "tr-TR";
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        <header className="mb-4">
          ...
          <p className="text-xs text-slate-400">
            Type or paste your text (up to 500 characters), choose languages, and let BhashaSetu AI handle the translation.
          </p>
        </header>

        {/* Input card */}
        <section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4 space-y-3 mb-4">
          <div className="flex flex-wrap items-center gap-3 justify-between">
            <div className="flex flex-wrap items-center gap-3">
              <div className="flex items-center gap-2">
                <label className="text-xs text-slate-400">From</label>
                <select
                  className="bg-slate-900 border border-slate-700 text-xs rounded-full px-3 py-1 focus:outline-none focus:ring-1 focus:ring-blue-500"
                  value={sourceLang}
              ...
              <button
                ...
                className="px-3 py-1 rounded-full text-xs font-medium bg-slate-800 hover:bg-slate-700"
              >
                Swap
              </button>
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  className="bg-slate-900 border border-slate-700 text-xs rounded-full px-3 py-1 focus:outline-none focus:ring-1 focus:ring-violet-500"
                  value={targetLang}
            ...
            <div className="text-[11px] text-slate-400 flex flex-col items-end">
              ...
              <span>Characters: {chars}/{MAX_CHARS}</span>
              <span>Estimated reading: {readingTime}</span>
            </div>
          </div>

          <div className="rounded-xl bg-slate-950 border border-slate-800">
            <textarea
              className="w-full h-40 resize-none rounded-xl bg-slate-950 px-3 py-3 text-sm text-slate-100 placeholder:text-slate-500 focus:outline-none focus:ring-1 focus:ring-blue-500 overflow-y-auto"
              placeholder="Type or paste text to translate (max 500 characters)..."
              value={inputText}
              onChange={(e) => {
                ...
                if (value.length <= MAX_CHARS) {
                  ...
                  setInputText(value.slice(0, MAX_CHARS));
                }
              }}
            />
          </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <button
              ...
              className="px-4 py-2 rounded-full text-sm font-medium bg-blue-600 hover:bg-blue-500 disabled:opacity-60"
        ...
        <section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4 space-y-3">
          ...
          <div className="rounded-xl bg-slate-950 border border-slate-800 h-40 overflow-y-auto px-3 py-3 text-sm text-slate-100">
            {outputText ? (
